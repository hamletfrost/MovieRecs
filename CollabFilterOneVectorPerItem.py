...
class CollabFilterOneVectorPerItem(AbstractBaseCollabFilterSGD):
    ...
    def predict(self, user_id_N, item_id_N,
        ...
        yhat_N = mu + b_per_user[user_id_N] + c_per_item[item_id_N] \
                + ag_np.einsum('ij->i',ag_np.einsum('ij,ij->ij',U[user_id_N], V[item_id_N]))
        return yhat_N
...
#        loss_total = ag_np.sum(ag_np.abs(y_N - yhat_N))
#        loss_total = ag_np.sum(ag_np.square(data_tuple[2] - self.predict(data_tuple[0], data_tuple[1], **param_dict))) \
#        loss_total = ag_np.sum(ag_np.square(data_tuple[2] - (param_dict['mu'] + param_dict['b_per_user'][data_tuple[0]] + param_dict['c_per_item'][data_tuple[1]] + \
#                                ag_np.einsum('ij->i',ag_np.einsum('ij,ij->ij',param_dict['U'][data_tuple[0]],param_dict['V'][data_tuple[1]]))))) \
        loss_total = ag_np.sum(ag_np.square(y_N - yhat_N)) \
                + (self.alpha * (ag_np.sum(ag_np.square(param_dict['U'])) \
